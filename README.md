...
| 언어 변경 | **설정 → 언어** |
| 내보내기 / 가져오기 | **설정 → 데이터 → 파일로 내보내기 / 파일에서 가져오기** |
...
파일 경로를 직접 찾기 번거롭다면 **설정 → 데이터** 에서 그대로 할 수 있습니다.
**파일로 내보내기** 는 위 내용을 전부 담은 JSON 파일을 원하는 위치에 저장하고
(`keep-it-real-2026-08-17.json` 처럼 날짜가 붙습니다), **파일에서 가져오기** 는 그 파일을
다시 읽어 들입니다. 가져오기는 파일을 고른 뒤 몇 건이 들어 있는지 보여주고 한 번 더 확인을
받고 나서야 적용되며, 적용되는 순간 지금 데이터를 **대체** 합니다 (합치지 않습니다).
위 표의 `keep-it-real.json` 을 그대로 골라도 읽히니, 다른 컴퓨터에서 복사해 온 파일도
그대로 쓸 수 있습니다. Keep It Real 데이터가 아닌 파일은 거부하고, 알아볼 수 없는 값은
버린 뒤 기본값으로 채웁니다.
